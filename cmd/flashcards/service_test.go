...
// TestBackwardCompatibility tests that the application can load and upgrade
// files from older versions of the application that don't have the due_dates field.
func TestBackwardCompatibility(t *testing.T) {
	// Create a temporary directory for the test
	tempDir := t.TempDir()
	testFilePath := filepath.Join(tempDir, "old-format.json")

	// Create a sample file in the old format (without due_dates field)
	oldFormatData := `{
		"cards": {
			"test-card-id": {
				"id": "test-card-id",
				"front": "Legacy Card Front",
				"back": "Legacy Card Back",
				"created_at": "2023-01-01T12:00:00Z",
				"tags": ["legacy", "old-format"],
				"last_reviewed_at": "2023-01-02T12:00:00Z",
				"fsrs": {
					"Due": "2023-01-03T12:00:00Z",
					"Stability": 1,
					"Difficulty": 0.5,
					"ElapsedDays": 1,
					"ScheduledDays": 1,
					...
					"Lapses": 0,
					"State": 2,
					"LastReview": "2023-01-02T12:00:00Z"
				}
			}
		},
		"reviews": [
	...
	// Write the old format data to the test file
	err := os.WriteFile(testFilePath, []byte(oldFormatData), 0644)
	if err != nil {
		t.Fatalf("Failed to create test file: %v", err)
	}

	// Create a new storage instance using the test file
	fileStorage := storage.NewFileStorage(testFilePath)
	...
	// Verify existing data was loaded correctly
	cards, err := fileStorage.ListCards(nil)
	if err != nil {
	...
	if cards[0].ID != "test-card-id" {
		t.Errorf("Expected card ID 'test-card-id', got '%s'", cards[0].ID)
	}
	if cards[0].Front != "Legacy Card Front" {
		t.Errorf("Expected card front 'Legacy Card Front', got '%s'", cards[0].Front)
	}

	// Check reviews were loaded correctly
	reviews, err := fileStorage.GetCardReviews("test-card-id")
	if err != nil {
	...
	if reviews[0].ID != "test-review-id" {
		t.Errorf("Expected review ID 'test-review-id', got '%s'", reviews[0].ID)
	}

	// Verify due_dates structure exists but is empty
	dueDates, err := fileStorage.ListDueDates()
	if err != nil {
		t.Fatalf("Failed to list due dates: %v", err)
	}
	if dueDates == nil {
		t.Error("DueDates should be initialized as empty slice, not nil")
	}
	if len(dueDates) != 0 {
		t.Errorf("Expected 0 due dates, got %d", len(dueDates))
	}

	// Add a new due date to verify the new structure works
	testDueDate := storage.DueDate{
		ID:      "test-upgrade-due-date",
		Topic:   "Upgraded Test",
		DueDate: time.Date(2024, 6, 1, 0, 0, 0, 0, time.UTC),
		Tag:     "test-upgrade-tag",
	}
	err = fileStorage.AddDueDate(testDueDate)
	if err != nil {
		t.Fatalf("Failed to add due date: %v", err)
	}

	// Save the file to upgrade format
	err = fileStorage.Save()
	if err != nil {
		t.Fatalf("Failed to save upgraded file: %v", err)
	}

	// Read the saved file and verify it contains the due_dates field
	fileData, err := os.ReadFile(testFilePath)
	if err != nil {
		t.Fatalf("Failed to read saved file: %v", err)
	}

	// Check that the file contains the due_dates field
	if !strings.Contains(string(fileData), "due_dates") {
		t.Error("Saved file does not contain 'due_dates' field")
	}

	// Check that our added due date is in the file
	if !strings.Contains(string(fileData), "test-upgrade-due-date") {
		t.Error("Saved file does not contain the added due date ID")
	}

	// Load the file again to verify everything is preserved across a fresh load
	newStorage := storage.NewFileStorage(testFilePath)
	err = newStorage.Load()
	if err != nil {
		t.Fatalf("Failed to load upgraded file: %v", err)
	}

	// Verify cards are still there
	cards, err = newStorage.ListCards(nil)
	if err != nil || len(cards) != 1 {
		t.Errorf("Card data not preserved across upgrade: %v", err)
	}

	// Verify due dates are there
	dueDates, err = newStorage.ListDueDates()
	if err != nil || len(dueDates) != 1 {
		t.Errorf("Due date not preserved: %v", err)
	}
	if len(dueDates) > 0 && dueDates[0].ID != "test-upgrade-due-date" {
		t.Errorf("Expected due date ID 'test-upgrade-due-date', got '%s'", dueDates[0].ID)
	}
}
